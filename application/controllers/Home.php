<?php 
	...
	public function index(){
		$data['data_kategori'] =$this->Produk_model->kategori();
		$data['all']=$this->Produk_model->produk_all();
		$this->load->view('roma/header',$data);
		$this->load->view('roma/home',$data);
		$this->load->view('roma/footer',$data);
	}
	...
	public function produk(){
		...
		$data['all']=$this->Produk_model->produk_all();
		$this->load->view('roma/header',$data);
		$this->load->view('roma/produk',$data);
		$this->load->view('roma/footer',$data);
	}
	...
	public function detail_barang($slug)
		{
			$data['data_kategori'] =$this->Produk_model->kategori();
			$apaja=($this->uri->segment(3))?$this->uri->segment(3):0;
			$data['detail']= $this->Produk_model->get_detail_produk($slug)->row_array();
			$this->load->view('roma/header',$data);
		$this->load->view('roma/detail_produk',$data);
		$this->load->view('roma/footer',$data);
		}
	public function beli(){
		if(!$this->session->has_userdata('customer_username')){
			redirect(base_url() . "Auth/login",$data);
		}else{
		$data= array(
		...
		$this->session->set_flashdata('flash','ditambahkan kedlam keranjang');
		redirect('Home');
	}
}
	public function data_belanja(){
		$this->load->view('roma/header');
		$this->load->view('roma/tampil_cart');
	...
	public function loginSubmit(){
		...
		$data['pria']=$this->Produk_model->produk_all();
		...
		if($this->form_validation->run()==false){
			...
			if ($this->Produk_model->cheklogin($_POST["customer_username"],$_POST["customer_password"])>0){
				...
				$data["loginerror"]="<div class='alert alert-danger'>Username dan password yang anda masukan salah</div>";
	...
	function kurirdata(){
		$iden = $this->Produk_model->view_ordering_limit('identitas','id_identitas','DESC',0,1)->row_array();
		...
		if(!empty($d['rajaongkir']['results'])){
			...
			$this->load->view('tab/kurirdata',$data);			
		}
	}
	...
	function checkouts(){
		if (isset($_POST['submit'])){
			if ($this->session->idp!=''){
				...
				if ($this->session->customer_username){
					$cek = $this->Produk_model->view_where('tr_order',array('session'=>$this->session->idp));
					if ($cek->num_rows()>=1){
						$data['title'] = 'Data Pelanggan';
						$data['kota'] = $this->model_app->view_ordering('rb_kota','kota_id');
						$data['rows'] = $this->model_app->view_join_where_one('rb_konsumen','rb_kota','kota_id',array('id_konsumen'=>$this->session->customer_username))->row_array();
